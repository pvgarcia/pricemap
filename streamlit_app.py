...
def load_financials(file) -> pd.DataFrame:
    ...
    fin[HUB_COL] = fin.apply(
        lambda row: _normalize_hub_value(row[ISO_COL], row[HUB_COL]),
        axis=1,
    )
...
def draw_map(
    ...
    for ridx, r in pts.iterrows():
        ...
        if sol_txt is not None:
            lines.append(f"Solar{stat_suffix}: {sol_txt}")
        if win_txt is not None:
            lines.append(f"Wind{stat_suffix}: {win_txt}")
...
with st.sidebar:
    ...
    st.markdown("### Solar/Wind statistic")
    stat_choice = st.radio(
        "Statistic for Solar/Wind prices",
        options=["Mean", "P50 (median)", "P25", "P75"],
        index=0,
    )
    stat_map = {
        "Mean": "mean",
    ...
    agg_method = stat_map[stat_choice]
    ...
    if fin_file is not None and selected_isos:
        labels_preview = prepare_labels(fin, selected_isos, developer, agg_method)
...
if fin_file is None:
    st.info("Upload Developer Financials file to begin.")
    st.stop()
...
labels = prepare_labels(fin, selected_isos, developer, agg_method)
