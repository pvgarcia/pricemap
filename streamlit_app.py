...
def conus_limits():
    # Shrink northern extent a bit so Alberta does not dominate
    t = Transformer.from_crs("EPSG:4326", TARGET_CRS, always_xy=True)
    x0, y0 = t.transform(-130, 24)
    x1, y1 = t.transform(-60, 55)  # was 60 before
    pad = 100_000
...
def _normalize_hub_value(iso: str, hub: str) -> str:
    ...
    if iso_clean == "AESO":
        if hub_clean.lower().startswith("alberta"):
            return "Alberta"
...
def load_financials(file) -> pd.DataFrame:
    ...
    for c in fin.columns:
        ...
        elif cl in {"hub", "settlement hub", "settlement_hub", "node", "settlement point", "settlement_point"}:
...
def prepare_labels(fin: pd.DataFrame, selected_isos: list[str], developer: str | None) -> pd.DataFrame:
    ...
    hubs = scoped.groupby([ISO_COL, HUB_COL], dropna=True).size().reset_index()[[ISO_COL, HUB_COL]]
...
def draw_map(
    ...
    # Then draw borders on top so they remain visible
    ...
    for ridx, r in pts.iterrows():
        ...
        if sol_txt is not None:
            lines.append(f"S: {sol_txt}")
        if win_txt is not None:
            lines.append(f"W: {win_txt}")
...
with st.sidebar:
    ...
    show_atc = st.checkbox("Show ATC price", value=True)
    show_solar = st.checkbox("Show Solar price", value=False)
    show_wind = st.checkbox("Show Wind price", value=False)
...
if fig is not None:
    ...
    st.download_button(
        ...
        data=png_buf,
        file_name="price_map_ATC_Solar_Wind_avg.png",
        mime="image/png",
    )
